...
void debug(struct eth_arp s_arp){
	...
	puts("");
	printf("arp t/araget_ip : %x\n",s_arp.a.target_ip);
}
/*
void getNetworkInfo(struct eth_arp *s_arp, char* interface) {
	char str[100]="ifconfig ";
	char *str_mac=" | grep ether | awk '{print $2}'";
	char *dummy=" && ifconfig ";
	char *str_ip= " | grep netmask | awk '{print $2}'";
	if (strstr(interface, ";") == NULL || strstr(interface, "|") == NULL) {
		exit(-1);
	}
	...
	for (; res[i]=='\n'; i++) {
		if (res[i] == ':') continue;
		// string mac --> byte mac (uint8_t [8]) conversion
	}
	inet_pton(AF_INET,res[i],(uint32_t*)s_arp.a.sender_ip);
	printf("%x\n",s_arp->a.sender_ip);
	for (int j=strlen(res), cipher=1; j > i; j--) {
		if (res[i] == '.') {
			// What is it?
			cipher = 1;
			continue;
		}
		// string ip --> byte ip (uint32_t) conversion
		cipher *= 10;
	}
...
int check_p(const u_char* packet2,struct eth_arp s_arp){
	...
	if(target->e.e_type==s_arp.e.e_type){
		if(memcmp(target->e.des_mac,"\xff\xff\xff\xff\xff\xff",6)!=-1){
				printf("BC\n");
			if(target->a.sender_mac==s_arp.a.sender_mac){
				debug(*target);
				return 0;
			}
		}
	...
	while(true){
		...
		if(check_p(packet2,argv.s_arp)!=-1){
			break;
  			...
  			pcap_sendpacket(handle,(unsigned char *)&argv.s_arp,42);
			continue;
		}
	}
	return NULL;	
...
int main(int argc, char* argv[]) {
  ...
  while(true){
	...
	if (res == 0) continue;
    	if (res == -1 || res == -2) break;
	memcpy(t_ip,(unsigned char *)packet2+28,4);
	memcpy(t_mac,(unsigned char *)packet2+22,6);
  ...
  inet_pton(AF_INET,argv[2],(uint32_t*)&s_arp.a.target_ip);
  pcap_sendpacket(handle,(const u_char *)&s_arp,42);
  while(1){
  	puts("123");
	res = pcap_next_ex(handle, &header, &packet2);	
	if (res == 0) continue;
    	if (res == -1 || res == -2) break;
  	puts("456");
	memcpy(s_ip,(unsigned char *)packet2+28,4);
	memcpy(s_mac,(unsigned char *)packet2+22,6);

	int ret = memcmp((char *)&s_ip,(char *)&s_arp.a.target_ip,4);
  	printf("%d\n",ret);
	break;
  }
  ...
  argvs.s_arp = s_arp;
  argvs.dev = dev;
  //if(pthread_create(&t_id,NULL,thread_main,(void *)&argvs)!=0){
//	puts("thread Error");
//	return -1;
 // }
  ...
  while(true){
	...
	if (res == 0) continue;
    	if (res == -1 || res == -2) break;
	if(t1->a.sender_mac==s_arp.a.target_mac){
		for(int i =0;i<sizeof(s_arp.e.src_mac);i++){
  			t1->e.src_mac[i]=s_arp.e.src_mac[i];
			t1->e.des_mac[i]=s_mac[i];
		}
		debug(*t1);
		pcap_sendpacket(handle,(unsigned char *)t1,header->caplen);
	}
  }
